...
##--------------------------------------------- Version 1.21 (dated December 12, 2016) --------------------------------------------------
...
############################################
usat<-function(Y, X, COV=NULL, na.check=TRUE, na.check.msg=TRUE, manova.out=FALSE, AbsTol=.Machine$double.eps^0.8){		
				# Y: continuous traits
  ...
  #### check for NA's and remove subjects (rows with at least one NA)
   ...
   # need to do the same for SSU score calculation else the score vector becomes NA
     ...
     if(na.check) 
     {
	   ...
	   if(na.check.msg) message("Excluding missing observations.")
	   ...
	   if(na.check.msg) message(paste("Current sample size:",nrow(Y)))
	}
     }
   ...
   #SumSqU:
    ...
    { #it's possible U=0 and Cov(U)=0:
        if (all(abs(U)<1e-20)) { Tssu<-0; pssu<-1 }
        else{
            Tssu<- t(U) %*% U
	...
	me <- sum(eig>0)
	 ...
	 c1 <- rep(0, 4)
      ...
      # i.e., pvalue is P(chi2_k>=quant|Ts)*f(Ts)
		...
		pval.T<-as.double(integrate(Vectorize(f.Liu.chi.upp),0,Inf,abs.tol=AbsTol)$value)
 }# end of else loop for det(TOT)=0

 if(manova.out) 
   return(list(T.manova=Tc, p.manova=pman, T.usat=pval.opt, omg.opt=rho.opt[1], p.usat=pval.T, n.obs=n))
 else
